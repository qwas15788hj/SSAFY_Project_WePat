...
public class PhotoController {
    ...
    public ResponseEntity<?> addCommentByPhotoId(@PathVariable("photoid") String photoId,
        ...
        try {
            return new ResponseEntity<>(photoService.addCommentByPhotoId(photoId, commentDto), HttpStatus.OK);
        } catch (NotExistImageException e) {
            throw new NotExistImageException();
        } catch (Exception e) {
            throw new RuntimeException(e);
    ...
    @DeleteMapping("/comment/{photoid}/{commentid}")
    @ApiOperation(value = "앨범 댓글 삭제")
    public ResponseEntity<?> deleteCommentByPhotoId(@PathVariable("photoid") String photoId,
                                                    @PathVariable("commentid") String commentId) {

        try {
            return new ResponseEntity<>(photoService.deleteCommentByPhotoId(photoId, commentId), HttpStatus.OK);
        } catch (NotExistImageException e) {
            throw new NotExistImageException();
        } catch (Exception e) {
            throw new RuntimeException(e);
    ...
    public ResponseEntity<?> updateCommentByPhotoId(@PathVariable("photoid") String photoId,
                                                    ...
                                                    @RequestBody CommentDto commentDto) {
        try {
            return new ResponseEntity<>(photoService.updateCommentByPhotoId(photoId, commentId, commentDto), HttpStatus.ACCEPTED);
        } catch (NotExistImageException e) {
            throw new NotExistImageException();
        } catch (Exception e) {
            throw new RuntimeException(e);
